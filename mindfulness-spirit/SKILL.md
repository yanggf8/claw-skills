...
- `persona_slug` (string, optional): persona-skill registry 中的 slug（如 `"ping-w"`）。設定後會從 persona-skill 載入 role、name、expression 等，優先於 `persona` 物件。未設定或 slug 不存在時 fallback 到 `persona` 物件。
